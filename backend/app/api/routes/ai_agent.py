...
def chat(
    ...
    db.add(AIMessage(conversation_id=conversation.id, role=ROLE_USER, content=payload.message))
    db.commit()
    ...
    try:
        ...
        logger.error("ai_sales_agent_reply_failed", organization_id=str(org_id), conversation_id=str(conversation.id))
        raise HTTPException(
